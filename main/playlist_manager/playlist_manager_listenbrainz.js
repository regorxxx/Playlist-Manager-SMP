...
ListenBrainz.importUserPlaylists = async function importUserPlaylists(user) {
	...
	if (!bDone) { fb.ShowPopupMessage('There were some errors on playlist syncing. Check console.', window.FullPanelName); }
	return bDone;
};
/**
...
ListenBrainz.decryptToken = function decryptToken({ lBrainzToken, bEncrypted = true }) {
	if (bEncrypted && !this.cache.key) {
		let pass = '';
		try { pass = utils.InputBox(window.ID, 'Enter password:', window.FullPanelName, pass, true); }
