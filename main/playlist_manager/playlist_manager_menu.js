		...
		{	// Load
			...
			menu.newEntry({
				entryText: (!bIsLockPls && !bIsLockPlsRename ? 'Rename...' : (bIsAutoPls || bIsPlsUI ? 'Rename...' : 'Rename... (only filename)')) + list.getGlobalShortcut('rename'), func: () => {
					const input = Input.string('string', pls.name, 'Enter playlist name:', window.FullPanelName + ': ' + pls.name, 'My playlist', void (0), true);
			...
			if (bIsAutoPls) {
				...
				menu.newEntry({
					entryText: 'Edit sort pattern...' + (bIsPlsUI ? '\t(cloning required)' : ''), func: () => {
						...
						if (pls.sort.length) { // And force sorting
							const bSortForced = pls.extension === '.xsp' ? false : WshShell.Popup('Force sort?\n(currently ' + pls.bSortForced + ')', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes;
				...
				menu.newEntry({
					entryText: 'Edit query...' + (bIsPlsUI ? '\t(cloning required)' : ''), func: () => {
						...
						let newQuery = '';
						try { newQuery = utils.InputBox(window.ID, 'Enter ' + (bXsp ? 'Smart Playlist' : 'AutoPlaylist') + ' query:', window.FullPanelName, pls.query); }
				...
				if (pls.extension === '.xsp') { // Smart Playlist Limit
					menu.newEntry({
						entryText: 'Edit limit...', func: () => {
							let input = Input.number('int positive', pls.limit, 'Enter number of tracks:', window.FullPanelName + ': ' + pls.name, 50);
							if (input === null) { return; }
				...
				if (pls.extension === '.ui') {
					...
					menu.newEntry({
						entryText: bIsLockPls ? 'Force playlist file update' : 'Update playlist file', func: () => {
							if (_isFile(pls.path)) {
					...
					menu.newEntry({
						entryText: bIsPlsActive ? 'Already bound to active playlist' : 'Bind active playlist to this file', func: () => {
							if (_isFile(pls.path)) {
				...
				{	// Export to ListenBrainz
					...
					menu.newEntry({
						menuName: subMenuName, entryText: 'Import from ListenBrainz' + (bListenBrainz ? '' : '\t(token not set)'), func: async () => {
							if (!pls.playlist_mbid.length) { return false; }
							if (!await checkLBToken()) { return false; }
							let bDone = false;
							if (_isFile(pls.path)) {
								const token = bListenBrainz ? lb.decryptToken({ lBrainzToken: list.properties.lBrainzToken[1], bEncrypted: list.properties.lBrainzEncrypt[1] }) : null;
									...
									.then((jspf) => {
										if (jspf) {
											...
											if (bXSPF) {
												...
												if (bDone && plman.FindPlaylist(pls.nameId) !== -1) { sendToPlaylist(new FbMetadbHandleList(handleArr.filter(Boolean)), pls.nameId); }
												if (bDone) {
					...
					menu.newEntry({
						menuName: subMenuName, entryText: 'Get URL...' + (pls.playlist_mbid ? '' : '\t(no MBID)'), func: async () => {
							console.popup('Playlist URL:\n\t ' + lb.getPlaylistURL(pls), window.FullPanelName);
...
function createFolderSubMenu(menu, folder, z) {
	...
	{ // New Playlists
		...
		menu.newSeparator(subMenuName);
		!list.bLiteMode && menu.newEntry({ menuName: subMenuName, entryText: 'New playlist from active...', func: () => { list.add({ bEmpty: false, toFolder: folder }); }, flags: plman.ActivePlaylist === -1 ? MF_GRAYED : MF_STRING });
		if (plman.ActivePlaylist !== -1 && plman.IsAutoPlaylist(plman.ActivePlaylist)) {
			menu.newEntry({
				...
				}, flags: plman.ActivePlaylist === -1 ? MF_GRAYED : MF_STRING
			});
		}
		menu.newEntry({
			menuName: subMenuName,
	...
	menu.newEntry({
		entryText: 'Rename...' + list.getGlobalShortcut('rename'), func: () => {
			const input = Input.string('string', folder.nameId, 'Enter folder name:', window.FullPanelName + ':' + folder.name, 'My folder', void (0), true);
			if (input === null) { return; }
	...
	{	// Tags and category
		if (showMenus['Category']) {	// Set category
			...
			menu.newEntry({
				menuName, entryText: 'New category...', func: () => {
					let category = '';
					try { category = utils.InputBox(window.ID, 'Category name (only 1):', window.FullPanelName, playlists[0].category === null ? '' : playlists[0].category, true); }
					catch (e) { return; } // eslint-disable-line no-unused-vars
					indexes.forEach((z, i) => {
						const pls = playlists[i];
			...
			menu.newEntry({
				menuName, entryText: 'New tag(s)...', func: () => {
					const tags = Input.json(
						...
						playlists[0].tags,
						'Tag(s) Name(s):\n(JSON)\n\nFor ex: ["Summer", "Cool"]',
						window.FullPanelName + ': ' + playlists.map((pls) => pls.name).joinUpToChars(', ', 50),
						'["TagA","TagB"]',
						void (0),
						true
					);
		...
		if (showMenus['Tags']) {	// Adds track tag(s)
			menu.newEntry({
				entryText: 'Automatically add tag(s) to tracks...', func: () => {
					...
					try { tags = utils.InputBox(window.ID, 'Enter data json-formatted: [{"tagName":"tagValue"}]\n\nTagValue may be:\n- String or number (doesn\'t need quotes).\n- TF expression applied to added track.\n- JS:+Function name (see helpers_xxx_utils.js).\n\nFor ex: [{"Mood":"Chill"}] or [{"Rating":5}]', window.FullPanelName, currValue, true); }
					catch (e) { return; } // eslint-disable-line no-unused-vars
					const tagsString = tags;
					if (tags.length) {
						tags = tags.replaceAll('\'\'', '"'); // Replace quotes
		...
		{	// Export and copy
			menu.newEntry({
				entryText: 'Export and Copy Tracks to...', func: () => {
					let path = '';
					try { path = sanitizePath(utils.InputBox(window.ID, 'Enter destination path:\n(don\'t forget adding \\ to copy to subfolder)', window.FullPanelName, list.playlistsPath + 'Export\\', true)); }
					catch (e) { return; } // eslint-disable-line no-unused-vars
					if (!path.length) { return; }
					if (path === list.playlistsPath) { console.log('Playlist Manager: can\'t export playlist(s) to original path.'); return; }
					const bSubFolder = WshShell.Popup('Create a subfolder per playlist?', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes;
...
function createListMenu() {
	...
	{ // New Playlists
		...
		menu.newSeparator();
		!list.bLiteMode && menu.newEntry({ entryText: 'New playlist from active...', func: () => { list.add({ bEmpty: false }); }, flags: plman.ActivePlaylist === -1 ? MF_GRAYED : MF_STRING });
		if (plman.ActivePlaylist !== -1 && plman.IsAutoPlaylist(plman.ActivePlaylist)) {
			menu.newEntry({
				entryText: 'New AutoPlaylist from active ...', func: () => {
					const pls = { name: plman.GetPlaylistName(plman.ActivePlaylist) };
		...
		if (showMenus['Online sync']) {
			menu.newEntry({
				...
				entryText: 'Import from ListenBrainz...' + (bListenBrainz ? '' : '\t(token not set)'), func: async () => {
					if (!await checkLBToken()) { return false; }
					let bDone = false;
					let playlist_mbid = Input.string('string', menu.cache.playlist_mbid || '', 'Enter Playlist MBID:', window.FullPanelName, '866b5a46-c474-4fae-8782-0f46240a9507', [(mbid) => isUUID(mbid.replace(lb.regEx, ''))]);
					...
					if (playlist_mbid.length) {
						...
						lb.importPlaylist({ playlist_mbid }, token)
							.then((jspf) => {
								if (jspf) {
									let bXSPF = false;
									if (list.playlistsExtension === '.xspf') { bXSPF = true; }
									else {
										const answer = WshShell.Popup('Save as .xspf format?\n(Items not found on library will be kept)', 0, window.FullPanelName, popup.question + popup.yes_no);
									...
									if (bXSPF) {
										...
										if (bDone && plman.FindPlaylist(playlistNameId) !== -1) { sendToPlaylist(new FbMetadbHandleList(handleArr.filter(Boolean)), playlistNameId); }
										if (bDone) {
											list.update({ bReuseData: false, bNotPaint: true, currentItemIndex: list.lastIndex });
											list.filter();
										}
												...
												return plman.AddPlaylistItemsOrLocations(idx, handleArr.filter(Boolean), true)
													.finally(() => {
														...
														if (_isFile(playlistPath)) {
															let answer = WshShell.Popup('There is a playlist with same name/path.\nDo you want to overwrite it?.', 0, window.FullPanelName, popup.question + popup.yes_no);
															if (answer === popup.no) { return false; }
		...
		{	// Import json
			...
			menu.newEntry({
				...
				entryText: 'Export playlists as json file...', func: () => {
					let answer = WshShell.Popup('Export only AutoPlaylists (yes) or both AutoPlaylists and other playlists -.fpl & .xsp- (no)?', 0, window.FullPanelName, popup.question + popup.yes_no);
		...
		if (!list.bLiteMode) {	// Size mismatch
			menu.newEntry({
				menuName: subMenuName, entryText: 'Playlist size mismatch...', func: () => {
					let answer = WshShell.Popup('Scan all playlists to check for reported playlist size not matching number of tracks.', 0, window.FullPanelName, popup.question + popup.yes_no);
					if (answer !== popup.yes) { return; }
		...
		if (!list.bLiteMode) {	// Duration mismatch
			menu.newEntry({
				menuName: subMenuName, entryText: 'Playlist duration mismatch...', func: () => {
					let answer = WshShell.Popup('Scan all playlists to check for reported playlist duration not matching duration of tracks.', 0, window.FullPanelName, popup.question + popup.yes_no);
					if (answer !== popup.yes) { return; }
					if (!pop.isEnabled()) { pop.enable(true, 'Checking...', 'Checking playlist duration mismatch...\nPanel will be disabled during the process.'); }
					findDurationMismatch().then(({ found, report }) => {
						if (found.length) { list.filter({ plsState: found }); }
						fb.ShowPopupMessage('Found these playlists with duration mismatch:\n\n' + (report.length ? report.join('\n') : 'None.') + (list.itemsFpl && report.length ? '\n\nAll .fpl playlists cached duration tags have also been updated.' : ''), window.FullPanelName);
						pop.disable(true);
					});
				}
			});
		...
		if (!list.bLiteMode) {	// Blank Lines
			menu.newEntry({
				menuName: subMenuName, entryText: 'Blank lines...', func: () => {
					let answer = WshShell.Popup('Scan all playlists to check for blank lines (it may break playlist on other players).', 0, window.FullPanelName, popup.question + popup.yes_no);
					if (answer !== popup.yes) { return; }
		...
		if (!list.bLiteMode) {	// Format specific errors
			menu.newEntry({
				menuName: subMenuName, entryText: 'Format specific errors...', func: () => {
					let answer = WshShell.Popup('Scan all playlists to check for errors on playlist structure or format.', 0, window.FullPanelName, popup.question + popup.yes_no);
					if (answer !== popup.yes) { return; }
		...
		if (!list.bLiteMode) {	// Circular references
			menu.newEntry({
				menuName: subMenuName, entryText: 'Circular references...', func: () => {
					let answer = WshShell.Popup('Scan all playlists to check for circular references on Smart Playlists (.xsp).', 0, window.FullPanelName, popup.question + popup.yes_no);
					if (answer !== popup.yes) { return; }
...
function createSettingsMenu(parent, parentBackground) {
	// Constants
	const z = (parent.index === -1) ? parent.getCurrentItemIndex() : parent.index;
	...
	if (!parent.bLiteMode) {	// Playlist folder
		menu.newEntry({
			entryText: 'Set playlists folder...', func: () => {
				...
				if (!bDone) {
					fb.ShowPopupMessage('Path can not be found or created:\n\'' + input + '\'', window.FullPanelName);
					return;
				}
				// Update property to save between reloads
				parent.properties['playlistsPath'][1] = input;
				parent.playlistsPath = _resolvePath(input);
				parent.playlistsPathDirName = parent.playlistsPath.split('\\').findLast(Boolean);
				parent.playlistsPathDisk = parent.playlistsPath.split('\\').find(Boolean).replace(':', '').toUpperCase();
				...
				presets.forEach((preset, i) => {
					...
					menu.newEntry({
						menuName: subMenuNameTwo, entryText: 'Set DSP preset...', func: () => {
							let input = Input.string('string', preset.dsp, 'Enter DSP preset name:\n(empty or ... will show converter window)', window.FullPanelName, 'my preset');
							if (input === null) { return; }
	...
	{	// UI
		...
		{	// List colors
			const subMenuName = menu.newMenu('Custom colors', menuName);
			{
				...
				const options = ['Standard playlists...', 'AutoPlaylists...', parent.bLiteMode ? null : 'Smart playlists...', parent.bAllPls ? 'UI-only playlists...' : null, 'Locked Playlists...', 'Selection rectangle...', showMenus['Folders'] ? 'Folders...' : null];
				options.forEach((item, i) => {
					if (!item) { return; }
					let key;
					switch (i) {
		...
		{	// Columns
			...
			parent.columns.labels.forEach((key, i) => {
				...
				{	// Size
					...
					menu.newCheckMenuLast(() => { const idx = options.indexOf(parent.columns.font[i]); return (idx === -1 ? 0 : idx); }, options.length);
				}
				{	// Align
					...
					menu.newCheckMenuLast(() => { const idx = options.indexOf(parent.columns.align[i]); return (idx === -1 ? 0 : idx); }, options.length);
				}
				{	// Color
					...
					menu.newCheckMenuLast(() => { const idx = options.indexOf(parent.columns.color[i]); return (idx === -1 ? (options.length - 1) : idx); }, options.length);
				}
				// Width
				menu.newEntry({
			...
			{	// Line
				...
				menu.newCheckMenuLast(() => { const idx = options.indexOf(parent.columns.line); return idx === -1 ? 0 : idx; }, options.length);
			}
			{	// Auto-Width
				...
				menu.newCheckMenuLast(() => { const idx = options.indexOf(parent.columns.autoWidth); return idx === -1 ? 0 : idx; }, options.length);
			}
			{	// Size unis
	...
	{	// Shortcuts
		...
		{	// List R. Click
			...
			modifiers.forEach((modifier) => {
				...
				actions.forEach((action) => {
					const flags = parent.isValidAction(action) ? MF_STRING : MF_GRAYED;
					menu.newEntry({
						menuName: subMenuOption, entryText: action, func: () => {
	...
	{	// Integration
		...
		{	// Startup active playlist
			...
			const name = idx === -1 ? nameUI : parent.dataAll[idx].name;
	...
	menu.newEntry({
		entryText: 'Lite mode', func: () => {
			...
			toMerge.forEach((d) => {
				d.file = d.from;
				d.from = _isFile(d.from) ? _jsonParseFileCheck(d.from, 'Data json', window.FullPanelName, utf8) : null;
			});
			...
			toMerge.forEach((d) => {
				if (d.from) {
					let bDone = false;
					d.to = _jsonParseFileCheck(d.to, 'Data json', window.FullPanelName, utf8);
					if (!d.to) {
						console.popup('Error restoring old playlists data from:\n' + d.file, window.FullPanelName);
						return;
					}
					switch (d.type) { // NOSONAR
						case 'pls': {
							for (const pls of d.from) {
								if (!d.to.some((oldPls) => parent.comparePls(oldPls, pls))) {
									d.to.push(pls);
									bDone = true;
								}
							}
	...
	{	// Filter
			...
			list.searchMethod.dragDropPriority.forEach((method, i) => {
				menu.newEntry({
					menuName: subMenuTwo, entryText: (i + 1) + '. ' + capitalize(method.replace(/^b/, '').replace(/MetaTracks/, 'track tags')) + (list.searchMethod[method] ? '' : '\t(disabled)'), func: () => {
						let input = Input.number('int positive', (i + 1), 'Enter position: (between 1 and ' + max + ')', window.FullPanelName, 1, [(n) => n >= 1 && n <= max]);
						if (input === null) { return; }
						else { input -= 1; }
		...
		menu.newEntry({
			menuName: subMenu, entryText: 'Path level matching...' + '\t' + _b(list.searchMethod.pathLevel), func: () => {
				let input = Input.number('int positive', list.searchMethod.pathLevel, 'Enter path level to search matches:', window.FullPanelName, 2);
				if (input === null) { return; }
...
async function checkLBToken(lBrainzToken = list.properties.lBrainzToken[1]) {
	if (!lBrainzToken.length) {
		...
		try { lBrainzToken = utils.InputBox(window.ID, 'Enter ListenBrainz user token:', window.FullPanelName, currToken, true); }
		...
		if (lBrainzToken.length) {
			...
			const answer = WshShell.Popup('Do you want to encrypt the token?', 0, window.FullPanelName, popup.question + popup.yes_no);
			if (answer === popup.yes) {
				let pass = '';
				try { pass = utils.InputBox(window.ID, 'Enter a password:\n(will be required on every use)', window.FullPanelName, pass, true); }
				catch (e) { return false; } // eslint-disable-line no-unused-vars
		...
		menu.newEntry({
			menuName: subMenuName, entryText: 'Cycle on last result', func: () => {
				...
				if (list.properties.bQuickSearchCycle[1]) {
					fb.ShowPopupMessage('Enabling this option will cycle between all the found items, not stopping on the last one but going back to the first one when no more items are found.', window.FullPanelName);
				}
			...
			importSettings(
				{
					...
					onUnzipData: (importPath, panelName) => { // eslint-disable-line no-unused-vars
						let bDone = getFiles(importPath, loadablePlaylistFormats)
							...
							.every((done) => {
								if (!done) {
									console.popup(panelName + ': failed importing playlist files.', window.FullPanelName);
									return false;
								}
								return true;
							});
						...
						bDone = bDone && getFiles(importPath, new Set(['.json', '.old']))
							...
							.every((done) => {
								if (!done) {
									console.popup(panelName + ': failed importing playlist json.', window.FullPanelName);
									return false;
								}
								return true;
							});
