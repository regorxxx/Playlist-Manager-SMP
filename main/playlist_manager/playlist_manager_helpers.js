...
function loadPlaylistsFromFolder(folderPath = '', bProfile = true) {
	const test = bProfile ? new FbProfiler(window.FullPanelName + ': ' + 'loadPlaylistsFromFolder') : true;
	...
	for (let i = 0; i < playlistPathArrayLength; i++) {
		...
		if (extension === '.m3u8' || extension === '.m3u') { // Schema does not apply for foobar2000 native playlist format
			...
			if (Object.hasOwn(jspf, 'playlist') && jspf.playlist) {
				...
				if (Object.hasOwn(jPls, 'meta') && Array.isArray(jPls.meta) && jPls.meta.length) {
					for (let metaData of jPls.meta) {
						...
						if (Object.hasOwn(metaData, 'playlistSize')) { size = typeof metaData.playlistSize === 'undefined' ? null : Number(metaData.playlistSize); }
						if (Object.hasOwn(metaData, 'duration')) { duration = typeof metaData.duration === 'undefined' ? null : round(Number(metaData.duration), 2); }
			...
			if (typeof text !== 'undefined') {
				...
				if (size > 1) { fb.ShowPopupMessage('.strm playlist can\'t contain multiple items: ' + file, window.FullPanelName); }
...
function convertToRelPaths(list, z) {
	let bDone = false;
	let answer = WshShell.Popup('Force relative paths to the playlist path, stripping all but the filenames.\nCan be manually undone (look at recycle bin). Are you sure?', 0, window.FullPanelName, popup.question + popup.yes_no);
	...
	if (file.length) {
		...
		if (bDeleted) {
			...
			if (bDone) { list.editData(pls, { fileSize: utils.GetFileSize(pls.path), modified: Date.now() }); }
				...
				fb.ShowPopupMessage('Playlist generation failed while writing file \'' + playlistPath + '\'.', window.FullPanelName);
			...
			fb.ShowPopupMessage('Playlist generation failed when overwriting original playlist file \'' + playlistPath + '\'. May be locked.', window.FullPanelName);
			return bDone;
		}
	}
	clearInterval(delay);
...
function cloneAsAutoPls(list, z, uiIdx = -1, toFolder = void (0)) { // May be used only to copy an Auto-Playlist or Smart Playlist
	...
	if (pls.extension === '.ui' && uiIdx !== -1) {
		WshShell.Popup('Native AutoPlaylists not created with the manager require cloning first to fully integrate them in the manager.\n\nThe AutoPlaylist properties will be shown to let you manually copy the query and sort patterns to the input popups. You can close it afterwards.', 5, window.FullPanelName, popup.info + popup.ok);
...
async function cloneAsStandardPls(list, z, opt = { remDupl: [], bAdvTitle: false, bMultiple: false }, bAddToList = true) { // May be used to copy an Auto-Playlist to standard playlist or simply to clone a standard one
	...
	if (idx && idx.length === 1) { // Already loaded? Duplicate it
		...
		if (newIdx === -1) { console.log('Error duplicating playlist'); return false; }
		else { plman.ActivePlaylist = newIdx; }
		bDone = true;
		...
		if (newIdx === -1) { console.log('Error duplicating playlist'); return false; }
		else { plman.ActivePlaylist = newIdx - 1; }
		bDone = true;
	} else {
	...
	const handleList = bUI
		...
		: pls.isAutoPlaylist
			...
			: pls.extension === '.fpl' && list.fplRules.bNonTrackedSupport
				...
				: getHandlesFromPlaylist({ playlistPath: pls.path, relPath: list.playlistsPath, bOmitNotFound: true }); // Omit not found
	if (handleList) {
...
function exportPlaylistFiles(list, zArr, defPath = '') {
	...
	let path = '';
	try { path = sanitizePath(utils.InputBox(window.ID, 'Enter destination path:\n(don\'t forget adding \\ to copy to subfolder)', window.FullPanelName, defPath || list.playlistsPath + 'Export\\', true)); }
	catch (e) { return bDone; } // eslint-disable-line no-unused-vars
...
function exportPlaylistFileWithRelPaths({ list, z, ext = '', defPath = '', bNoInput = false } = {}) {
	...
	if (bNoInput) {
		...
		try { newPath = sanitizePath(utils.InputBox(window.ID, 'Enter destination path:', window.FullPanelName, defPath.length ? defPath + playlistName : list.playlistsPath + 'Export\\' + playlistName, true)); }
	...
	if (file.length) {
		...
		if (bDeleted) {
			...
			if (!bDone) {
				fb.ShowPopupMessage('Playlist generation failed while writing file \'' + newPath + '\'.', window.FullPanelName);
...
function exportPlaylistFileWithTracks({ list, z, defPath = '', bAsync = true, bNoInput = false, bOpenOnExport = true } = {}) {
	...
	} else { fb.ShowPopupMessage('Failed when copying playlist file to \'' + newPath + '\'.', window.FullPanelName); }
	return bDone;
}
...
function exportPlaylistFileWithTracksConvert({ list, z, tf = '.\\%FILENAME%.mp3', preset = '...', defPath = '', ext = '', playlistOutPath = '', remDupl = [], bAdvTitle = false, bMultiple = false, bExtendedM3U = true } = {}) {
	...
	const handleList = bUI
		? getHandlesFromUIPlaylists([pls.nameId], false)
		: getHandlesFromPlaylist({ playlistPath: pls.path, relPath: list.playlistsPath, bOmitNotFound: true, remDupl, bAdvTitle, bMultiple }); // Omit not found
	const subsongRe = subsongRegex;
	...
	if (bUI) {
		...
		paths.forEach((trackPath, i) => {
			if (_isFile(trackPath.replace(subsongRe, ''))) { newHandleList.Add(handleList[i]); }
			else { report.push(trackPath); }
		});
		...
		if (newName.length && newName !== oldName) {
			if (pls.isAutoPlaylist || pls.extension === '.ui') {
				...
				if (_isFile(pls.path)) { // NOSONAR
					...
					bRenamedSuccessfully = oldPath === newPath ? true : _renameFile(oldPath, newPath);
...
function renameFolder(list, z, newName) {
	...
	if (list.dataAll.some((pls) => pls.nameId === newName)) {
		fb.ShowPopupMessage('Name already used: ' + newName + '\n\nChoose an unique name for renaming.', window.FullPanelName);
		return false;
	} else {
					...
					if (!bFpl && typeof text !== 'undefined' && text.length) {
						...
						if (playlist.extension === '.m3u8' || playlist.extension === '.m3u') {
							...
							if (Object.hasOwn(jspf, 'playlist') && jspf.playlist) {
								...
								if (Object.hasOwn(jPls, 'meta') && Array.isArray(jPls.meta) && jPls.meta.length) {
									for (let metaData of jPls.meta) {
										if (Object.hasOwn(metaData, 'playlistSize')) {
											size = typeof metaData.playlistSize === 'undefined' ? null : Number(metaData.playlistSize);
										}
									}
								}
							}
							...
							if (!bFpl && typeof text !== 'undefined' && text.length) {
								...
								if (playlist.extension === '.m3u8' || playlist.extension === '.m3u') {
									...
									if (Object.hasOwn(jspf, 'playlist') && jspf.playlist) {
										...
										if (Object.hasOwn(jPls, 'meta') && Array.isArray(jPls.meta) && jPls.meta.length) {
											for (let metaData of jPls.meta) {
												if (Object.hasOwn(metaData, 'duration')) {
													duration = typeof metaData.playlistSize === 'undefined' ? null : Number(metaData.duration);
												}
											}
										}
									}
						...
						? bUI
							...
							: playlist.isAutoPlaylist
								...
								: playlist.extension === '.fpl' && list.fplRules.bNonTrackedSupport
									? getHandlesFromPlaylistV2({ playlistPath: playlist.path, bOmitNotFound: true, bReturnNotFound: true, poolItems: libItems })
										.then((out) => out.handlePlaylist)
									: getHandlesFromPlaylist({ playlistPath: playlist.path, relPath: list.playlistsPath, bOmitNotFound: true, poolItems: libItems }) // Omit not found
						: null
				)
					.then((handleList) => Promise.wait(15).then(() => handleList))
