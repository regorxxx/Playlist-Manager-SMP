...
function ThemedPanelButton(
	...
	this.draw = function (gr, bLast = false) {
		...
		if (this.icon !== null) {
			...
			gr.GdiDrawText(iconCalculated, this.gFontIcon, buttonsPanel.config.textColor, xCalc + offset, yCalc, textWidthCalculated, hCalc, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX); // Icon
...
function on_mouse_lbtn_down_buttn(x, y) { // eslint-disable-line no-unused-vars
...
function on_mouse_lbtn_up_buttn(x, y) { // eslint-disable-line no-unused-vars
	buttonsPanel.gDown = false;
...
function addButton(newButtons) {
	...
	for (let buttonName in newButtons) {
		if (Object.hasOwn(buttonsPanel.buttons, buttonName)) {
