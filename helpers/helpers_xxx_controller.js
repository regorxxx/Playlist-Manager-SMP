...
function exportDSP(path) {
	...
	return _save(path + 'dsp.json', JSON.stringify(data, null, '\t').replace(/\n/g, '\r\n'));
}

function exportDevices(path) {
	const data = JSON.parse(fb.GetOutputDevices()); // Reformat with tabs
	return _save(path + 'devices.json', JSON.stringify(data, null, '\t').replace(/\n/g, '\r\n'));
}
function exportComponents(path, newEntries /*{key: val, ...*/) {
	...
	return _save(path + 'components.json', JSON.stringify(data, null, '\t').replace(/\n/g, '\r\n'));
}